...
```sh
uv run python main.py <input.ply> [options]
```
...
```sh
uv run python main.py scan.ply -o topo_map.png
```

DXF for Vectorworks with simplified contours:
```sh
uv run python main.py scan.ply -o topo_map.dxf --simplify 0.3
```

Smoothed ground with outlier filtering:
```sh
uv run python main.py scan.ply -o topo_map.png --smooth 2.0 --filter 3
```
