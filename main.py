#!/usr/bin/env python3
"""Convert a Scaniverse 3D scan export into a top-down topographic contour map."""
...
def filter_outliers(points: np.ndarray, strength: float, bin_count: int = 50) -> np.ndarray:
    ...
    print(f"Outlier filter (strength={strength:.1f}): removed {n_removed} of {len(points)} points "
...
def compute_grid(points, grid_resolution, smooth):
    ...
    xi = np.linspace(x_ft.min() - margin * x_range, x_ft.max() + margin * x_range, grid_resolution)
    yi = np.linspace(y_ft.min() - margin * y_range, y_ft.max() + margin * y_range, grid_resolution)
    xi_grid, yi_grid = np.meshgrid(xi, yi)

    print(f"Interpolating {len(points)} points onto {grid_resolution}x{grid_resolution} grid...")
    ...
    if smooth > 0:
        ...
        zi_grid[mask] = np.nan
        print(f"Applied Gaussian smoothing: sigma = {smooth:.1f} ft ({sigma_cells:.1f} grid cells)")
...
def compute_levels(zi_grid, contour_interval_ft):
    ...
    index_interval = 5.0
    index_levels = levels[np.isclose(levels % index_interval, 0, atol=contour_interval_ft * 0.1)]
...
def _extract_contours(xi_grid, yi_grid, zi_grid, levels, simplify_tolerance):
    ...
    if simplify_tolerance > 0:
        print(f"Simplified contours: {total_before} → {total_after} vertices "
              f"({100 * (1 - total_after / total_before):.0f}% reduction, tolerance={simplify_tolerance:.1f} ft)")

    return contours


def export_dxf(xi_grid, yi_grid, zi_grid, levels, index_levels, output_path,
...
def render_png(
    ...
    x_range, y_range, output_path, dpi, simplify_tolerance=0,
):
    ...
    cf = ax.contourf(
        ...
        levels=levels, cmap="YlGn", norm=norm, alpha=0.5,
    )
    ...
    if len(index_levels) > 0:
        for level_val, seg in contours:
            ...
            if level_rounded in index_set and len(seg) > 4:
                mid = seg[len(seg) // 2]
                ax.annotate(
                    f"{level_val:.0f}'", xy=(mid[0], mid[1]),
                    fontsize=7, color="#3d2010", ha="center", va="center",
...
def _add_scale_bar(ax, x_range_ft, y_range_ft):
    ...
    ax.add_patch(plt.Rectangle(
        (x0, y0), bar_len, bar_height,
        facecolor="black", edgecolor="black", linewidth=0.5, zorder=10,
    ))
    tick_h = bar_height * 3
    for bx in [x0, x0 + bar_len]:
        ax.plot([bx, bx], [y0, y0 + tick_h], color="black", linewidth=0.8, zorder=10)
    ax.text(
        x0 + bar_len / 2, y0 + tick_h + (ylim[1] - ylim[0]) * 0.005,
        f"{bar_len} ft",
